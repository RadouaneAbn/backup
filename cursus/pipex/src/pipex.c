...
{
	if (ft_strchr(cmd_v[0], '/') == NULL || file_exists(cmd_v[0]) == 0)
	{
...
// 				child_execute(env, i, commands, fd[1], in, path);
// 			}
// 			else if (i == cmd_count - 1)
// 			{
// 				out = open(av[ac - 1], O_CREAT | O_WRONLY | O_TRUNC, 0644);
// 				child_execute(env, i, commands, out, fd[0], path);
// 			}
// 			else
// 				child_execute(env, i, commands, fd[1], prev, path);
// 		}
// 		if (prev != -1)
// 			close(prev);
// 		prev = fd[0];
