...
void Bureaucrat::signForm(Form &form) const
{
	...
	{
		form.beSigned(*this);
		std::cout << BLUE << _name << RESET << " signed " << GREEN << form.getName() << RESET << std::endl;
	}
	catch (std::exception &e)
	{
		std::cout << BLUE << _name << RESET << " couldn't sign " << RED << form.getName() << RESET << " because " << e.what() << std::endl;
	}
}
